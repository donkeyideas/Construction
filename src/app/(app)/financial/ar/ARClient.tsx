...
export default function ARClient({
  ...
  async function handleToggleGLSources() {
    ...
    if (typeof window !== "undefined") {
      if (next) {
        localStorage.setItem("buildwrk_ar_gl_sources", "true");
        document.cookie = "buildwrk_use_gl_ar_ap=true;path=/;max-age=31536000";
      } else {
        localStorage.removeItem("buildwrk_ar_gl_sources");
        // Only clear cookie if AP toggle is also off
        if (!localStorage.getItem("buildwrk_ap_gl_sources")) {
