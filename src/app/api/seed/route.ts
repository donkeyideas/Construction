...
export async function POST(request: Request) {
  try {
    ...
    await supabase.from("tickets").insert([
      ...
      { company_id: companyId, ticket_number: `TK-${String(ticketNum++).padStart(4, "0")}`, title: "QuickBooks integration sync error", description: "Invoice sync between Buildwrk and QuickBooks failed last night. 5 invoices from Feb 10 not showing in QB.", status: "open", priority: "high", category: "IT", created_by: userIds.accountant, assigned_to: userIds.owner, tags: ["integration", "quickbooks", "invoices"] },
