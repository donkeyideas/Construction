...
/**
 * Same as getGLBalanceForAccountType but only includes JE lines
 * with entry_date <= cutoffDate. Used by cash flow to compute
 * period-start and period-end balances from GL.
 */
export async function getGLBalanceAsOfDate(
  supabase: SupabaseClient,
  companyId: string,
  accountType: string,
  namePattern: string,
  mode: "debit-credit" | "credit-debit",
  cutoffDate: string
): Promise<number> {
  const { data: accounts } = await supabase
    .from("chart_of_accounts")
  ...
  for (;;) {
    ...
    if (error) {
      console.error("GL balance as-of-date query error:", error.message);
      break;
    }
    if (!lines || lines.length === 0) break;
