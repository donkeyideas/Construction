...
export async function generateInvoiceJournalEntry(
  ...
  const isDeferred = !!(invoice.deferral_start_date && invoice.deferral_end_date);

  // Idempotency: check if a JE already exists for this invoice
  ...
  if (existingJEs && existingJEs.length > 0) {
    if (options?.forceRegenerate) {
      // Delete all existing JEs for this invoice so we can recreate correctly
      const jeIds = existingJEs.map((je: { id: string }) => je.id);
      ...
      // Return existing JE (idempotent)
      return { journalEntryId: existingJEs[0].id };
    }
  }
  ...
  if (invoice.invoice_type === "receivable") {
    ...
    // For deferred revenue: credit Deferred Revenue liability instead of Revenue directly.
    // Monthly recognition JEs (generated by generateInvoiceDeferralSchedule) move
    // the balance from Deferred Revenue → Revenue each period.
    ...
    // CR: Deferred Revenue (if deferred) or Revenue account
    lines.push({
      account_id: creditAccountId,
      debit: 0,
      credit: subtotal,
      description: isDeferred ? `Deferred: ${description}` : description,
      project_id: invoice.project_id ?? undefined,
      property_id: invoice.property_id ?? undefined,
    });
